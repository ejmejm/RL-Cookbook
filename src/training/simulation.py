...
def train_exploration_model(agent: BaseAgent, env, n_steps, print_rewards=True, print_freq=5000):
  ...
  while step_idx < n_steps:
    ...
    # # Uncomment to log every timestep
    # wandb.log({
    ...
    if done:
      ...
      wandb.log({
        ...
        'env_explore_act_entropy': act_entropy,
        'env_explore_reward': reward,
        'env_explore_episode': episode_idx,
        'env_explore_episode_reward': ep_reward})
      ep_reward = 0
    else:
      obs = next_obs
  ...
  while step_idx < n_steps:
    ...
    if done:
      ...
      wandb.log({
        'env_task_step': step_idx,
        'env_task_act_entropy': act_entropy,
        'env_task_episode': episode_idx,
        'env_task_episode_reward': np.sum(episode_rewards)})
      episode_rewards = []
      episode_idx += 1
    else:
      obs = next_obs
